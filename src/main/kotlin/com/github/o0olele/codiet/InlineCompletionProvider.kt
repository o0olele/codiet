 ...
 */
class CodietInlineCompletionProvider : InlineCompletionProvider {
    /**
    ...
    override suspend fun getSuggestion(request: InlineCompletionRequest): InlineCompletionSuggestion {
        ...
        return InlineCompletionSuggestion.Default(
            channelFlow {
                ...
                val suggestion = (if (prefix in CodietCache) {
                    CodietCacheStatistic.onCacheHit()
                    CodietCache[prefix]
                } else if (lastPrefixLine in CodietCache) {
                    CodietCacheStatistic.onCacheHit()
                    CodietCache[lastPrefixLine]
                } else {
                    CodietCacheStatistic.onCacheMiss()
                    OpenAILLM.call(prefix, suffix)?.also {
                        addCurrentToCache(prefix, it)
                        addCurrentToCache(lastPrefixLine, it)
                    }
                } ?: "").let { CodietStatisticAnalyzer.makeSingleLineIfNeeded(it) }
                ...
                CodietStatistic.onCompletion(duration)
            }
        )
    }
    ...
    private fun addCurrentToCache(prefix: String, currentSuggestion: String) {
        if (prefix.isNotBlank()) {
            CodietCache[prefix] = currentSuggestion
        }
    }
    ...
    private fun addNextToCache(prefix: String, suffix: String, currentSuggestion: String) {
        ...
        if (nextPrefix !in CodietCache) {
            OpenAILLM.call(nextPrefix, suffix)?.also {
                if (it.isNotBlank()) {
                    CodietCache[nextPrefix] = it
                }
            }
        }
    }
